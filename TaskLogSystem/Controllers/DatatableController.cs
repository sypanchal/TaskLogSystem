    ...
    public class DatatableController : Controller
    {
        ...
        public JsonResult LoadEmployeeTaskData(int EmployeeID)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(searchValue))
                {
                    ...
                    x.TaskDate.ToLower().Contains(searchValue.ToLower()) ||
                    x.ApproverName.ToLower().Contains(searchValue.ToLower()) ||
                    x.ApprovedorRejectedByName.ToLower().Contains(searchValue.ToLower()) ||
                    x.ApprovedorRejectedOn.ToLower().Contains(searchValue.ToLower()) ||
                    x.Status.ToLower().Contains(searchValue.ToLower()) ||
                    x.CreatedOn.ToLower().Contains(searchValue.ToLower()) ||
                    x.ModifiedOn.ToLower().Contains(searchValue.ToLower())
                    );
                }
        ...
        public JsonResult LoadEmployeesData()
        {
            ...
            try
            {
                ...
                // Getting all Employees data for the current user (Manager or Director)
                var list = Emp.RoleID == 2 ?
                    _dbContext.Employees.Where(t => t.ReportingPerson == Emp.EmployeeID && !t.IsDeleted).ToList()
                    : _dbContext.Employees.Where(t => !t.IsDeleted && t.EmployeeID != 101).ToList();
                ...
                var formattedList = list.Select(t => new
                {
                    ...
                    t.Email,
                    t.Role.RoleName,
                    ReportingPersonName = t.Employee1.FirstName + " " + t.Employee1.LastName,
                }).AsQueryable();
        ...
        public JsonResult LoadAllTasksData()
        {
            ...
            try
            {
                ...
                // Getting all Employees' Task data
                var list = Emp.RoleID == 1 ? _dbContext.Tasks.Where(t => !t.IsDeleted).ToList() : _dbContext.Tasks.Where(t => t.ApproverID == Emp.EmployeeID && !t.IsDeleted).ToList();
